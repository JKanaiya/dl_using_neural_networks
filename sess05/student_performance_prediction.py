...
def build_neural_network(input_dimension: int) -> Sequential:
    ...
    model = Sequential(
        [
            ...
            Dense(
                32, activation="relu", kernel_regularizer=l2(REGULARIZATION_STRENGTH)
            ),
            Dense(
                16, activation="relu", kernel_regularizer=l2(REGULARIZATION_STRENGTH)
            ),
...
# 11. Utility/Summary
# -----------------------------------------------------------------------------
def display_summary(train_count: int, test_count: int, accuracy: float) -> None:
    """
    Purpose:
...
def main() -> None:
    ...
    try:
        ...
        display_prediction_examples(
            id_test, y_test.to_numpy(), predictions, probabilities
        )
