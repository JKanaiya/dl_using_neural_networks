"""
=======================================================================================
Python script to demonstrate Feedforward Neural Network for Maize yield prediction
=======================================================================================
...
REQUIRED_COLUMNS = [
    ...
    "Temp_M4",
    "Yield",
]  # Columns in the rainfall_temp_vs_yeild.csv file
...
def validate_dataset(df: pd.DataFrame) -> None:
    ...
    if df["Yield"].isna().all():
        raise ValueError("Yield column contains no values")
...
def prepare_data(
    ...
    df["Total_Rainfall"] = df["Rain_M1"] + df["Rain_M2"] + df["Rain_M3"] + df["Rain_M4"]

    df["Average Temperature"] = df[
        [
            "Temp_M1",
            "Temp_M2",
            "Temp_M3",
...
def evaulate_model(model: MLPRegressor, x_test: np.ndarray, y_test: pd.Series) -> tuple:
...
def plot_training_history(
    ...
    plt.close()  # plt.show to display the graph in pycharm
    print(train_losses)
...
def plot_residuals(actual: pd.Series, predicted: np.ndarray) -> None:
    ...
    plt.savefig(RESULTS_DIR / "residual_plot.png", dpi=300)
    plt.close
...
def main() -> None:
    try:
        ...
        print("Creting correlatin matrix...")
        create_correlation_matrix(df)

        print("Preparing features")
        x, y = prepare_data(df)

        print("Splitting datset...")
        ...
        mae, rmse, r2, predictions = evaulate_model(model, x_test_scaled, y_test)
        ...
        print("\n---------------------------------------")
        print("Maize yield prediction results")
        print("\n---------------------------------------")
