...
def create_results_directory() -> None:
    try:
        ...
        raise OSError(f"Unable to create results directory: {error}")
...
def validate_dataset(dataset: pd.DataFrame) -> None:
    ...
    if dataset[REQUIRED_COLUMNS].isnull.any():
...
def plot_confusion_matrix(y_true: np.ndarray, y_pred: np.ndarray) -> None:
    try:
        ...
        plt.savefig(RESULTS_DIR / "student_performance_confusion_matrix.png")
        plt.close()
    except Exception as e:
...
def display_prediction_examples(
    ...
    print("\n---- PREDICTION EXAMPLES (First 10 Students) ----")
    ...
    try:
        ...
        raise RuntimeError(f"Failed to save prediction examples plot: {e}") from e
...
def split_dataset(
    ...
    target: pd.Series,
    student_ids: pd.Series,
) -> tuple[np.np.ndarray, np.ndarray, pd.Series, pd.Series, pd.Series, pd.Series]:
...
def build_neural_network(input_dimension: int) -> Sequential:
    ...
    # optimisation, adjusting each weights learning rate individually using estimates
    # of first and second moments of gradeints.
...
def train_model(
    model: Sequential,
    x_train: np.ndarray,  # Fixed the type hint typo 'np.ndarry' -> 'np.ndarray'
    y_train: pd.Series
) -> tf.keras.callbacks.History:
    try:
        # 1. Compile the model if not already compiled
        # Note: Adjust loss/optimizer based on your specific dataset requirements
        if not model.compiled:
            model.compile(
                optimizer='adam',
                loss='sparse_categorical_crossentropy', # Change to 'binary_crossentropy' or 'mse' if needed
                metrics=['accuracy']
            )
        
        # 2. Convert pandas Series to numpy array if required by your pipeline configuration
        ...
        history = model.fit(
            x_train,
            y_train_arr,
            epochs=10,               # Adjust baseline epochs as necessary
            batch_size=32,           # Adjust batch size based on memory requirements
            validation_split=0.2,    # Standard 20% validation monitoring track
            verbose=1
        )
...
def evaluate_model(
    ...
    x_test: np.ndarray,
    y_test: pd.Series
) -> tuple[np.ndarrary, np.ndarray, float]:
    probabilities = model.predict(x_test, verbose=0)
    predictions = (probabilities > 0.5).astype(int).falatten()
...
# -----------------------------------------------------------------------------
def predict_student_outcomes(model: Sequential, x_test: np.ndarray) -> np.ndarray:
    return model.predict(x_test, verbose=0)

# TODO: DISPLAY SUMMARY FUNC MISSING
...
def main() -> None:
    try:
        ...
        scaled_features, _scaler = scale_features(features)
        
        x_train, x_test, y_train, y_test, id_train, id_test = split_dataset(scale_features, target, dataset["student_id"])
        
        model = build_neural_network(input_dimension=x_train[1])
